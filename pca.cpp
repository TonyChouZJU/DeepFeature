...
int main(int argc, char** argv)
{
    ...
    cout << "image nums:" << images.size() << "image rows:" << images[0].rows <<"image cols:" << images[0].cols <<endl; 
    // Reshape and stack images into a rowMatrix
    Mat data = formatImagesForPCA(images);
    cout << "data feature:"<< data.rows << " " << data.cols <<endl; 
    

    // perform PCA
    PCA pca(data, cv::Mat(), CV_PCA_DATA_AS_ROW, 100); // trackbar is initially set here, also this is a common value for retainedVariance

    // Demonstration of the effect of retainedVariance on the first image
    cout << " before projection,row(0).rows "<< data.row(0).rows << " " << data.row(0).cols<< endl;
    Mat point = pca.project(data.row(0)); // project into the eigenspace, thus the image becomes a "point"
    cout << " after projection, point "<< point.rows << " " << point.cols<< endl;
    Mat reconstruction = pca.backProject(point); // re-create the image from the "point"
    cout << " after projection, reconstruction "<< reconstruction.rows << " " << reconstruction.cols<< endl;
