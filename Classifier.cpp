...
std::vector<std::vector<Prediction>> Classifier::Classify(const std::vector<cv::Mat> &imgs, int N) {
    ...
    std::vector<std::vector<float> > output_vector=Predict(imgs);

    N = std::min<int>(labels_.size(), N);
    for(int k=0;k<imgs.size();++k){
        std::vector<float> output= output_vector[k];
        std::vector<int> maxN = Argmax(output, N);
        std::vector<Prediction> predictions;
        for (int i = 0; i < N; ++i) {
            int idx = maxN[i];
...
void Classifier::WrapInputLayer(std::vector<std::vector<cv::Mat>> &input_channels_vector) {
    Blob<float>* input_layer = net_->input_blobs()[0];
...
void Classifier::Preprocess(const std::vector<cv::Mat> &imgs, std::vector<std::vector<cv::Mat>> &input_channels_vector) {
