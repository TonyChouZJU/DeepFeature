...
def pencil(img, threshold=10):
    img_orig = img.copy().astype(np.int32)
    dst_img = np.ones(img.shape) * 255
    img = Image.fromarray(img.astype(np.uint8))
    matrix33 = tuple(np.array([1, 1, 1, 1, 0, 1, 1, 1, 1], dtype=np.float32)/8)
    img_mean = np.array(img.filter(ImageFilter.Kernel((3, 3), matrix33)), dtype=np.int32)
    sub_arrary = np.abs(img_orig - img_mean)
    r_idx,  c_idx = np.where((sub_arrary[:, :, 0] >= threshold) &
                             (sub_arrary[:, :, 1] >= threshold) &
                             (sub_arrary[:, :, 2] >= threshold)
                             )
    dst_img[r_idx, c_idx, :] = [0, 0, 0]
    return dst_img
'''
    ...
    for h in xrange(height):
        for w in xrange(width):
            if w == 0 or w == width - 1 \
                ...
                continue
            mean_arround = np.sum(img[h-1: h+2, w-1: w+2, :], axis=(0, 1))
            mean_arround -= img[h, w, :]
            mean_arround /= 8
            mean_arround = mean_arround.astype(np.int32)
            # print mean_arround
            isDraw = (abs(img[h, w, 0] - mean_arround[0]) >= threshold) & \
                     (abs(img[h, w, 1] - mean_arround[1]) >= threshold) & (abs(img[h, w, 2] - mean_arround[2]) >= threshold)

            if isDraw:
                dst_img[h, w, :] = [0, 0, 0]
            else:
                dst_img[h, w, :] = [255, 255, 255]
    return dst_img



def pencil_0(img, threshold):
...
if __name__ == "__main__":
    ...
    img = pencil_0(img, threshold)
    img_save = Image.fromarray(np.uint8(img))
    img_save.save(os.path.splitext(path)[0]+'.pencil_2.jpg', 'JPEG')
